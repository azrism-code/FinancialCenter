...
if(firebaseConfig.projectId.startsWith('REPLACE_')){
 ...
 window.portfolioStore={save(portfolio){if(!user)return Promise.reject(Error('Sign in required'));const destination=ref(),snapshot=JSON.parse(JSON.stringify(portfolio));for(const section of snapshot.portfolios||[]){section.marketProvider=snapshot.marketProvider||'auto';section.theme=snapshot.theme==='dark'?'dark':'light'}delete snapshot.marketProvider;delete snapshot.theme;for(const section of snapshot.portfolios||[])for(const item of section.items||[]){for(const prefix of ['history','intraday'])if(['Yahoo','Tiingo'].includes(item[prefix+'Source']))for(const key of Object.keys(item).filter(k=>k.startsWith(prefix)))delete item[key]}saveQueue=saveQueue.catch(()=>{}).then(()=>setDoc(destination,{...snapshot,updatedAt:Date.now()}));return saveQueue}};
